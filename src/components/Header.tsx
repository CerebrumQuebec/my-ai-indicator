...
const Header: React.FC<HeaderProps> = ({ showWizard, setShowWizard }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-surface-dark/80 backdrop-blur-md border-b border-white/10 shadow-lg">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          <Link
            href="/"
            onClick={handleReset}
            className="flex items-center space-x-3 text-2xl font-bold text-text-primary hover:text-primary-400 transition-all duration-300"
          >
            <div className="relative w-10 h-10 bg-primary-600/20 rounded-full p-1 flex items-center justify-center border border-primary-500/30 shadow-glow">
              <Image
                src="/logo.png"
                alt="Badge AI Logo"
                fill
