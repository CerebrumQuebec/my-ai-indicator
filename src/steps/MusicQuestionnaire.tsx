...
import {
  StepProps,
  musicQuestions,
  musicQuestionWeights,
  calculateCategory,
} from "../types";

const MusicQuestionnaire: React.FC<StepProps> = ({ onNext, onBack }) => {
  const {
    setMusicCategory,
  ...
  const handleOptionSelect = (questionId: string, points: number) => {
    const newAnswers = { ...musicQuestionnaireAnswers, [questionId]: points };
    setMusicQuestionnaireAnswers(newAnswers);
    ...
    if (currentQuestion < musicQuestions.length - 1) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-text-secondary">
          Question {currentQuestion + 1} sur {musicQuestions.length}
        </p>
      </div>
          ...
          {question.options.map((option) => (
            ...
            >
              <div className="mr-3 mt-1 w-5 h-5 flex-shrink-0 rounded-full border border-primary-500 flex items-center justify-center">
                {musicQuestionnaireAnswers[question.id] === option.points && (
      ...
      <div className="w-full bg-surface-card/50 h-1.5 rounded-full overflow-hidden">
        <div
          ...
          style={{
            width: `${((currentQuestion + 1) / musicQuestions.length) * 100}%`,
          }}
        ></div>
      </div>
    </div>
